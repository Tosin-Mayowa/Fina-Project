...
namespace WebApplication2.Pages.ControlPannel.SuperUser.Uploads.TBill
{
    public class IndexModel : PageModel
    {
        public bool Is_Done = true;
        public int Num { get; set; }
        private readonly Repository.InterfaceTBill _loanupload;
        private IConfiguration _configuration;
        private IWebHostEnvironment _webHostEnvironment;
        public ICollection<TBillFmdq> LoanUploadFields { get; set; }
        private readonly ApplicationDbContext _db;
        private readonly WebApplication2.Data.ApplicationDbContext _context;

        public IndexModel(WebApplication2.Repository.InterfaceTBill loanupload, IConfiguration configuration, Data.ApplicationDbContext context, ApplicationDbContext db)
        {
        ...
        public async Task<IActionResult> OnPostAsync(IFormFile formFile)
        {
            ...
            var num = await _context.TBills.ToListAsync();
            Num = num.ToList().Count();
            Is_Done = false;
            if (Num != 0)
            {
                await _db.Database.ExecuteSqlRawAsync("TRUNCATE TABLE TBills");
                ...
                using (SqlConnection scon = new SqlConnection(sqlconn))
                {
                    ...
                    using (var commd = new SqlCommand("Select count(*) from TBills", scon))
                    {
